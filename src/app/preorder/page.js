...
export default function PreOrderPage() {
  ...
  const updateStatus = async (id, field, currentVal) => {
    ...
    // Update State Lokal dulu agar instan di mata Admin
    ...
    try {
      ...
      alert("Gagal sinkron ke database, silakan refresh.");
      refreshData(); 
    }
  };

  // CHECKOUT DENGAN AUTO-UPDATE HISTORY
    ...
    try {
      for (const item of keranjang) {
        ...
        if (res.ok) {
          ...
          newItemsForHistory.push(result.preorder); // Ambil data lengkap dengan ID dari Sheety
        }
      }

      // Gabungkan data baru ke urutan paling atas riwayat secara instan
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
          ...
          <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 h-fit">
            ...
            <form onSubmit={(e) => {
              ...
              <select required value={selectedBarang ? selectedBarang.namaBarang : ""} disabled={!selectedVendor} className="w-full p-3 rounded bg-slate-900 border border-slate-700 disabled:opacity-30" 
